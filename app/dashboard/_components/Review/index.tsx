  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-4 pb-8">
      ...
      <div className="flex justify-between items-center mt-4 relative">
        {eventList && (eventList?.currentPage > 0 ? <button onClick={handlePrevPage}>
          <AiOutlineArrowLeft size={25} className="btn-anim text-luxtix-1 absolute left-0" />
        </button> : <></>)}
        {eventList && (eventList?.currentPage < eventList.totalPages - 1 ? <button onClick={handleNextPage}>
          <AiOutlineArrowRight size={25} className="btn-anim text-luxtix-1 absolute right-0" />
        </button> : <></>)}
      </div>
    </div>
  );
